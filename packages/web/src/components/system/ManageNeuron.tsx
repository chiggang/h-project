 ...
 * @constructor
 */
const ManageNeuron: React.FC<any> = (props: any) => {
  // 환경 설정을 정의함
  ...
  // 선택한 그룹에 속한 뉴런 데이터를 정의함
  const [selectedNeurons, setSelectedNeurons] = useState<INeuron[]>([]);
    ...
    axios({
      ...
      .then((response: AxiosResponse<any>) => {
        if (response.headers.authorization !== undefined) {
          ...
          // 선택한 그룹에 속한 뉴런을 불러옴
          let tmpNeurons2: INeuron[] = tmpNeurons.filter(
            (filterData: INeuron) =>
              filterData.group === selectedNeuronGroup?.id,
          );

          // 선택한 그룹에 속한 뉴런 데이터를 기억함
          setSelectedNeurons(
            _.cloneDeep(_.sortBy(tmpNeurons2, ['sortNoInGroup'])),
          );
        } else {
          // // 경고창을 출력함
          // Dialog.alert({
  ...
  const saveNeuronSort = () => {
    ...
    // 선택한 그룹이 변경되면 그룹에 속한 뉴런 데이터를 갱신함
    let tmpAllNeurons: INeuron[] = _.cloneDeep(neurons);

    let newCount: number = 1;

    selectedNeurons.map((neuron: INeuron) => {
      tmpAllNeurons.map((subNeuron: INeuron) => {
        if (neuron.id === subNeuron.id) {
          subNeuron.sortNoInGroup = newCount;
          newCount += 1;
        }
      });
    });
    ...
    // 뉴런 데이터를 기억함
    setNeurons(tmpAllNeurons);

    // 뉴런 순서를 수정함
    axios({
      method: 'PATCH',
      url: `${config.api.commonUrl.host}:${config.api.commonUrl.port}/neuron/neuron_sort`,
      headers: {
        // Authorization: userMutate?.authorization || '',
      },
      data: tmpAllNeurons,
      ...
      .then((response: AxiosResponse<any>) => {
        if (response.headers.authorization !== undefined) {
          ...
          console.log('> modify neuron sort error');
        }
      })
      .catch((error) => {
        console.log('> axios sort error:', error.response);
      });
  };
  ...
  // 선택한 그룹이 변경되면 그룹에 속한 뉴런 데이터를 갱신함
  const refreshSelectedNeuronsInGroup = () => {
    ...
    // 선택한 그룹에 속한 뉴런을 불러옴
    let tmpNeurons: INeuron[] = neurons.filter(
      (filterData: INeuron) => filterData.group === selectedNeuronGroup?.id,
    );

    // 선택한 그룹에 속한 뉴런 데이터를 기억함
    setSelectedNeurons(_.cloneDeep(_.sortBy(tmpNeurons, ['sortNoInGroup'])));
  };
  ...
  // 선택한 그룹이 변경될 때, 선택한 그룹에 속한 뉴런 데이터도 갱신함
  useEffect(() => {
    if (selectedNeuronGroup?.id === undefined) {
      return;
    }

    // 선택한 그룹이 변경되면 그룹에 속한 뉴런 데이터를 갱신함
    refreshSelectedNeuronsInGroup();
  }, [selectedNeuronGroup?.id]);
      ...
      <div className="space-y-5">
        <div className="flex justify-center items-start space-x-6">
          ...
          <div className="flex-none w-64 select-none">
            ...
            >
              {neuronGroup.map((group: INeuronGroup) => (
                <div
                  key={group.id}
                  className={`px-2 py-2 flex justify-between items-center border-b border-gray-200 hover:bg-indigo-100/20 ${
                    selectedNeuronGroup?.id === group.id
                      ? 'bg-indigo-100/50'
                      : 'bg-white'
                  }`}
                >
          ...
          <div className="grow select-none">
            ...
            {/* 뉴런 목록 */}
            <ReactSortable
              group="selectedNeurons"
              list={selectedNeurons}
              setList={setSelectedNeurons}
              handle=".drag-handle"
              animation={300}
              delayOnTouchOnly={true}
              delay={100}
            ...
            >
              {selectedNeurons.map((neuron: INeuron) => (
                <div
                  key={neuron.id}
                  onClick={() =>
                    !isNeuronEditMode && handleNeuronTableRow_onClick(neuron)
                  }
                ...
                >
                  ...
                  <div className="flex-none w-20 whitespace-normal">
                    <span className="text-xs font-normal text-gray-800">
                      {getNeuronGroupName(neuron.group)}
                    </span>
                  </div>
                  ...
                  {/* 이미지 */}
                  <div className="flex-none w-14">
                    {neuron.image && neuron.image.length > 0 && (
                      <FontAwesomeIcon
                        icon={['fas', 'check']}
                  ...
                  {/* PDF */}
                  <div className="flex-none w-10">
                    {neuron.pdf && neuron.pdf.length > 0 && (
                      <FontAwesomeIcon
                        icon={['fas', 'check']}
                        className="w-3 h-3 text-lime-600"
                      />
